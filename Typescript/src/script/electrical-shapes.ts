...
export class ElectricalShapes {
    ...
    private getElectricalAudioShapes(): NodeModel[] {
        ...
        for (const auto of audioShapes) {
            if(!auto.children) {
                auto.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            auto.addInfo = { type: 'CustomShapes' };
        }
        return audioShapes;
    }

    private getElectricalCapcitorsShapes(): NodeModel[] {
        const capacitorsShapes: NodeModel[] = [
            {
        ...
        for (const capacitor of  capacitorsShapes) {
            if(!capacitor.children) {
                capacitor.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            capacitor.addInfo = { type: 'CustomShapes' };
        }
        return capacitorsShapes;
    }

    private getElectricalDiodesShapes(): NodeModel[] {
        const diodeShapes: NodeModel[] = [
            {
        ...
        for (const diode of  diodeShapes) {
            if(!diode.children) {
                diode.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            diode.addInfo = { type: 'CustomShapes' };
        }
        return diodeShapes;
    }

    private getElectricalGatesShapes(): NodeModel[] {
        const gateShapes: NodeModel[] = [
            {
        ...
        for (const gate of  gateShapes) {
            if(!gate.children) {
                gate.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            gate.addInfo = { type: 'CustomShapes' };
        }
        return gateShapes;
    }

    private getElectricalInductorsShapes(): NodeModel[] {
        const inductorsShapes: NodeModel[] = [
            {
        ...
        for (const inductors of  inductorsShapes) {
            if(!inductors.children) {
                inductors.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            inductors.addInfo = { type: 'CustomShapes' };
        }
        return inductorsShapes;
    }

    private getElectricalInstrumentsShapes(): NodeModel[] {
        const instrumentShapes: NodeModel[] = [
            {
        ...
        for (const instrument of  instrumentShapes) {
            if(!instrument.children) {
                instrument.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            instrument.addInfo = { type: 'CustomShapes' };
        }
        return instrumentShapes;
    }

    private getElectricalMiscShapes(): NodeModel[] {
        const miscShapes: NodeModel[] = [
            {
        ...
        for (const misc of  miscShapes) {
            if(!misc.children) {
                misc.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            misc.addInfo = { type: 'CustomShapes' };
        }
        return miscShapes;
    }

    private getElectricalOpticalShapes(): NodeModel[] {
        const opticalShapes: NodeModel[] = [
            {
                id: 'SHAPE_LED2ycpV3b',
                shape: {
                    type: 'Path',
        ...
        for (const optical of  opticalShapes) {
            if(!optical.children) {
                optical.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            optical.addInfo = { type: 'CustomShapes' };
        }
        return opticalShapes;
    }

    private getElectricalLadderShapes(): NodeModel[] {
        const ladderShapes: NodeModel[] = [
            {
        ...
        for ( const ladder of ladderShapes) {
            if(!ladder.children) {
                ladder.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            ladder.addInfo = { type: 'CustomShapes' };
        }
        return ladderShapes;
    }

    private getElectricalResistorShapes(): NodeModel[] {
        const resistorShapes: NodeModel[] = [
            {
        ...
        for (const resistor of resistorShapes) {
            if(!resistor.children) {
                resistor.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            resistor.addInfo = { type: 'CustomShapes' };
        }
        return resistorShapes;
    }

    private getElectricalEquipmentShapes(): NodeModel[] {
        const equipmentShapes: NodeModel[] = [
            {
        ...
        for (const equipment of  equipmentShapes) {
            if(!equipment.children) {
                equipment.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            equipment.addInfo = { type: 'CustomShapes' };
        }
        return equipmentShapes;
    }

    private getElectricalSourcesShapes(): NodeModel[] {
        const sourcesShapes: NodeModel[] = [
            {
                id: 'SHAPE_AC2wodMyq',
                shape: {
                    type: 'Basic',
        ...
        for (const source of  sourcesShapes) {
            if(!source.children) {
                source.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            source.addInfo = { type: 'CustomShapes' };
        }
        return sourcesShapes;
    }

    private getElectricalSwitchesShapes(): NodeModel[] {
        const switchesShapes: NodeModel[] = [
            {
        ...
        for (const shape of  switchesShapes ) {
            if(!shape.children) {
                shape.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            shape.addInfo = { type: 'CustomShapes' };
        }
        return switchesShapes;
    }

    private getElectricalTransmissionShapes(): NodeModel[] {
        const transmissionShapes: NodeModel[] = [
            {
        ...
        for (const terminate of  transmissionShapes) {
            if(!terminate.children) {
                terminate.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            terminate.addInfo = { type: 'CustomShapes' };
        }
        return transmissionShapes;
    }

    private getElectricalTransistorsShapes(): NodeModel[] {
        const transistorsShapes: NodeModel[] = [
            {
        ...
        for (const transistor of  transistorsShapes) {
            if(!transistor.children) {
                transistor.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            transistor.addInfo = { type: 'CustomShapes' };
        }
        return transistorsShapes;
    }

    private getElectricalVaccumShapes(): NodeModel[] {
        const vaccumShapes: NodeModel[] = [
            {
        ...
        for (const vaccum of vaccumShapes) {
            if(!vaccum.children) {
                vaccum.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            vaccum.addInfo = { type: 'CustomShapes' };
        }
        return vaccumShapes;
    }

    private getElectricalWaveformsShapes(): NodeModel[] {
        const WaveformsShapes: NodeModel[] = [
            {
        ...
        for (const Waveform of  WaveformsShapes) {
            if(!Waveform.children) {
                Waveform.constraints = NodeConstraints.Default & ~NodeConstraints.Select;
            }
            Waveform.addInfo = { type: 'CustomShapes' };
        }
