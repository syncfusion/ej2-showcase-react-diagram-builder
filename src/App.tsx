...
import {
    ...
    UndoRedo, DiagramContextMenu, Snapping, DataBinding, PrintAndExport, BpmnDiagrams, HierarchicalTree, MindMap as MindMapTree, ConnectorBridging, LayoutAnimation, SymbolPalette, ConnectorEditing
...
import {
    ToolbarComponent, ItemsDirective, ItemDirective, ClickEventArgs, MenuEventArgs as ContextMenuEventArgs,
     MenuAnimationSettingsModel, ContextMenuComponent, OpenCloseMenuEventArgs
...
class App extends React.Component<{}, {}> {
    ...
    public segmentEdit:CheckBoxComponent;
    ...
    constructor(props: any) {
        ...
        segmentEditChange = this.connectorSegmentEditChange.bind(this);
    ...
    public removeSelectedToolbarItem(): void {
        for (const i of (this.toolbarEditor as any).items.length) {
            const item: any = (this.toolbarEditor as any).items[i];
    ...
    public drawShapeTemplate(): JSX.Element {
        return (
            <div id="template_toolbar">
                <DropDownButtonComponent id="btnDrawShape" ref={btnDrawShape => (this.btnDrawShape) = (btnDrawShape as DropDownButtonComponent)} items={this.dropDownDataSources.drawShapesList} iconCss='sf-icon-DrawingMode' select={drawShape} />
            </div>
    ...
    public drawConnector(): JSX.Element {
        return (
            <div id="template_toolbar">
                <DropDownButtonComponent id="btnDrawConnector" ref={btnDrawConnector => (this.btnDrawConnector) = (btnDrawConnector as DropDownButtonComponent)} items={this.dropDownDataSources.drawConnectorsList} iconCss='sf-icon-ConnectorMode' select={drawConnector} />
            </div>
        );
    }
    ...
    public connectorSegmentEditChange(args:any){
       const diagram: Diagram = this.selectedItem.selectedDiagram as Diagram;
       if(diagram.selectedItems.connectors){
         if(args.checked === true){
           for(let i=0;i<diagram.selectedItems.connectors.length;i++){
             const connector: any = (diagram.selectedItems as any).connectors[i];
                            ...
                            <div className='db-property-editor-container' style={{ overflow: "auto" }}>
                                <div id="generalDiagramContainer" className="db-general-diagram-prop-container">
                                    ...
                                    <div id='connectorPropertyContainer' className="db-connector-prop-container" style={{ display: "none" }}>
                                        ...
                                        <div className="row db-prop-row">
                                        <CheckBoxComponent ref={segmentEdit => (this.segmentEdit as CheckBoxComponent) = (segmentEdit as CheckBoxComponent)} id='segmentEditing'
                                                    label="Segment Editing" checked= {false} change={segmentEditChange} />
                                        </div>
                                        <div className="row db-prop-row">
                                            <div className="col-xs-2 db-col-right db-prop-text-style" style={{ paddingTop: "6px" }}>
                                                <span className="db-prop-text-style">Opacity</span>
